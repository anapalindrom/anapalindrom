...
def show_study_input():
    ...
    def start_study_session():
        ...
        def update_timer_and_arduino():
            global is_timing, start_time, study_line, has_saved  # Adăugăm variabila `has_saved`

            if not hasattr(update_timer_and_arduino, "has_saved"):
             update_timer_and_arduino.has_saved = False  # Inițializăm `has_saved` la prima rulare
...
def show_task_completion():
    ...
    # Center the task completion window
    ...
    def save_task_completion(value):
        ...
        task_completion = value  # 1 for Yes, 0 for No
        print("Task completion:", "Yes" if value == 1 else "No")
        task_window.destroy()  # Close the task window

        show_study_input()  # Return to the study input page or add the back button directly
...
def show_mood_tracker():
    ...
    top_frame.grid(row=0, column=0, pady=(50, 20), sticky="n")  # Utilizează sticky="n" pentru a poziționa în partea de sus
    ...
    # Create a new window for mood entry
    window = tk.Toplevel(container)
    window.title("Enter Mood")

    # Center the mood window
    ...
    instructions_text = (
        ...
        "1. Energized - Feeling highly accomplished and motivated, ready for more.\n"
        "2. Confident - Satisfied with the progress and confident in understanding.\n"
        "3. Satisfied - Content with the effort and results, feeling productive.\n"
        "4. Neutral - The session was average with no strong feelings.\n"
        "5. Disappointed - Unsatisfied with the progress, feeling like more could have been done.\n"
        "6. Frustrated - Upset due to difficulties encountered or lack of productivity.\n"
        "7. Exhausted - Completely burnt out by the session."
    )
    ...
    # Validation and entry for mood
    def validate_input(value):
      return value == "" or (value.isdigit() and 1 <= int(value) <= 7)

    # Înregistrăm și aplicăm noua funcție de validare în câmpul de introducere pentru *Mood*
    ...
    def save_mood():
        ...
        window.destroy()
        show_task_completion()  # Call task completion prompt after mood is saved

    save_button = tk.Button(window, text="Save Mood", command=save_mood)
    save_button.pack(pady=10)

    # Add a Back button to return to the study input
